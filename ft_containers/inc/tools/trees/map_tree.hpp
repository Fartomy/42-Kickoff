...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/14 14:33:12 by ftekdrmi          #+#    #+#             */
/*   Updated: 2023/02/14 14:33:13 by ftekdrmi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
    ...
    public:
        tree(const value_compare &comp) : alloc_(node_allocator()), value_alloc_(allocator_type()), comp_(comp), end_node_(), size_(0) { begin_iter_ = end_node(); }
    ...
    public:
        ...
        size_type max_size() const { return std::min(alloc_.max_size(), static_cast<size_type>(std::numeric_limits<difference_type>::max())); }
		...
		// -*-*-*-*-*-*-*-*-*-*-*-*-*-/ Operation Functions \-*-*-*-*-*-*-*-*-*-*-*-*-*-
        template <typename Key> size_type count(const Key &key) const { return find_pointer(key) == NULL ? size_type(0) : size_type(1); }
        template <typename Key> iterator find(const Key &key) { return find_key<iterator>(key); }
        template <typename Key> const_iterator find(const Key &key) const { return find_key<const_iterator>(key); }
        template <typename Key> pair<iterator, iterator> equal_range(const Key &key)
        {
            pair<end_node_pointer, end_node_pointer> range = eq_range(key);
            return ft::make_pair(iterator(range.first), iterator(range.second));
        }
        template <typename Key> pair<const_iterator, const_iterator> equal_range(const Key &key) const
        {
            pair<end_node_pointer, end_node_pointer> range = eq_range(key);
            return ft::make_pair(const_iterator(range.first), const_iterator(range.second));
        }
        template <typename Key> iterator lower_bound(const Key &key) { return iterator(low_bound(key)); }
        template <typename Key> const_iterator lower_bound(const Key &key) const { return const_iterator(low_bound(key)); }
        template <typename Key> iterator upper_bound(const Key &key) { return iterator(up_bound(key)); }
        template <typename Key> const_iterator upper_bound(const Key &key) const { return const_iterator(up_bound(key)); }

    private:
	// -*-*-*-*-*-*-*-*-*-*-*-*-*-/ Tools \-*-*-*-*-*-*-*-*-*-*-*-*-*-
        template <typename Iter, typename Key> Iter find_key(const Key &key) const
        {
        ...
        template <typename Key> end_node_pointer low_bound(const Key &key) const
        {
        ...
        template <typename Key> end_node_pointer up_bound(const Key &key) const
        {
            node_pointer ptr = root();
            end_node_pointer pos = end_node();
            while (ptr != NULL)
        ...
        template <typename Key> pair<end_node_pointer, end_node_pointer> eq_range(const Key &key) const
        {
            node_pointer ptr = root();
        ...
        node_pointer root() const { return end_node()->left; }
        node_pointer *root_ptr() const { return &(end_node()->left); }

        end_node_pointer end_node() { return static_cast<end_node_pointer>(&end_node_); }
        end_node_pointer end_node() const { return const_cast<end_node_pointer>(&end_node_); }
        ...
        template <typename Key> end_node_pointer find_pointer(const Key &key) const
        {
            node_pointer ptr = root();
            while (ptr != NULL)
            {
        ...
        template <typename Key> node_pointer &find_pos(end_node_pointer &parent, const Key &key) const
        {
        ...
        template <typename Key> node_pointer &find_pos(iterator hint, end_node_pointer &parent, const Key &key, node_pointer &dummy) const
        {
