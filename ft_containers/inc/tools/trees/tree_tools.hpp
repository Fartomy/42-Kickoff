...
#include "tree_structers.hpp"

namespace ft
{
	template <typename NodePtr> inline bool tree_is_left_child(NodePtr ptr) { return ( ptr == ptr->parent->left ); }

	template <typename NodePtr> NodePtr tree_max(NodePtr ptr)
	{
	...
	template <typename NodePtr> NodePtr tree_min(NodePtr ptr)
	{
		while (ptr->left != NULL)
			ptr = ptr->left;
		return ptr;
	...
	template <typename IterPtr, typename NodePtr> IterPtr tree_iter_next(NodePtr ptr)
	{
		if( ptr->right != NULL )
		{
	...
	template <typename NodePtr, typename IterPtr> IterPtr tree_iter_prev(IterPtr ptr)
	{
		if( ptr->left != NULL )
		{
	...
	template <typename NodePtr> void tree_rotate_left(NodePtr node)
	{
	...
	template <typename NodePtr> void tree_rotate_right(NodePtr node)
	{
	...
	template <typename NodePtr> void tree_rotate_left(NodePtr &root, NodePtr node)
	{
		if(node == root)
		{
			root = node->right;
	...
	template <typename NodePtr> void tree_rotate_right(NodePtr &root, NodePtr node)
	{
		if(node == root)
		{
			node = node->left;
	...
	template <typename NodePtr> inline bool tree_node_is_black(NodePtr node)
	{
	...
	template <typename NodePtr> void tree_insert_fix(NodePtr root, NodePtr z)
	{
		z->is_black = z == root;
	...
	template <typename NodePtr> void tree_delete_fix(NodePtr root, NodePtr x_parent)
	{
		NodePtr x = NULL;
	...
	template <typename NodePtr> void tree_transplant_node(NodePtr pos, NodePtr& node)
	{
	...
	template <typename NodePtr> void tree_remove_node(NodePtr root, NodePtr target)
	{
		NodePtr y = target;
